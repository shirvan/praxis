...
|-----------|------------|---------|-------------|
| CLI | Random hex ID, auto-persisted to `~/.praxis/session` | `a3f8c2d1e5...` | Automatic — saved after each invocation, reused by subsequent commands. `--new-session` starts fresh; `--session <id>` switches explicitly. |
...
praxis concierge configure --provider openai --base-url http://localhost:11434/v1 --model gemma4:26b
...
# Session management
praxis concierge status                  # uses auto-resolved session
praxis concierge status --session <id>
praxis concierge history --session <id>
praxis concierge reset                   # resets auto-resolved session
praxis concierge reset --session <id>

# Start a fresh session
praxis "what's running in prod?" --new-session
