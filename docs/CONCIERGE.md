...
> **See also:** [Architecture](ARCHITECTURE.md) | [Orchestrator](ORCHESTRATOR.md) | [Templates](TEMPLATES.md) | [CLI](CLI.md) | [Events](EVENTS.md) | [Drivers](DRIVERS.md) | [Slack Gateway](SLACK_GATEWAY.md)
...
| Slack DM (planned) | `slack:<team_id>:<user_id>` | `slack:T01ABC:U04XYZ` | Automatic — every DM from the same user hits the same session |
| Slack thread (planned) | `slack:thread:<channel_id>:<thread_ts>` | `slack:thread:C01ABC:1711234567.000100` | Automatic — the thread is the session |
...
| Slack (planned) | Gateway posts an interactive message with Approve/Reject buttons immediately after calling `Ask`. `Ask` is still running (suspended). | Button click calls `ApprovalRelay.Resolve`. The suspended `Ask` resumes, executes the tool, and returns the final response. |
...
| Slack DM (planned) | The same user whose DM session triggered the approval | Gateway checks user identity before calling the relay |
| Slack thread (planned) | Any user on the allow-list who is in the channel | Gateway checks the allow-list |
...
|---------|-----------|--------|---------------|
| Slack | `praxis-slack` | Planned | [Slack Gateway](SLACK_GATEWAY.md) |
| Discord | `praxis-discord` | Future | — |
