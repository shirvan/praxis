...
func ComputeFieldDiffs(desired IAMUserSpec, observed ObservedState) []drivers.FieldDiff {
	...
	if !stringSetEqual(desired.Groups, observed.Groups) {
		diffs = append(diffs, drivers.FieldDiff{Path: "spec.groups", OldValue: sortedStrings(observed.Groups), NewValue: sortedStrings(desired.Groups)})
	}
