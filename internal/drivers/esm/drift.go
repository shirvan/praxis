...
func ComputeFieldDiffs(desired EventSourceMappingSpec, observed ObservedState) []drivers.FieldDiff {
	var diffs []drivers.FieldDiff
	if desired.Enabled && observed.State == "Disabled" {
		diffs = append(diffs, drivers.FieldDiff{Path: "spec.enabled", OldValue: false, NewValue: true})
	}
	if !desired.Enabled && observed.State == "Enabled" {
		diffs = append(diffs, drivers.FieldDiff{Path: "spec.enabled", OldValue: true, NewValue: false})
	}
	if desired.BatchSize != nil && *desired.BatchSize != observed.BatchSize {
		diffs = append(diffs, drivers.FieldDiff{Path: "spec.batchSize", OldValue: observed.BatchSize, NewValue: *desired.BatchSize})
	}
	if desired.MaximumBatchingWindowInSeconds != nil && *desired.MaximumBatchingWindowInSeconds != observed.MaximumBatchingWindowInSeconds {
		diffs = append(diffs, drivers.FieldDiff{Path: "spec.maximumBatchingWindowInSeconds", OldValue: observed.MaximumBatchingWindowInSeconds, NewValue: *desired.MaximumBatchingWindowInSeconds})
	}
	if !filterCriteriaMatch(desired.FilterCriteria, observed.FilterCriteria) {
		diffs = append(diffs, drivers.FieldDiff{Path: "spec.filterCriteria", OldValue: observed.FilterCriteria, NewValue: desired.FilterCriteria})
	}
	if desired.BisectBatchOnFunctionError != nil && *desired.BisectBatchOnFunctionError != observed.BisectBatchOnFunctionError {
		diffs = append(diffs, drivers.FieldDiff{Path: "spec.bisectBatchOnFunctionError", OldValue: observed.BisectBatchOnFunctionError, NewValue: *desired.BisectBatchOnFunctionError})
	}
	if !int32PtrMatch(desired.MaximumRetryAttempts, observed.MaximumRetryAttempts) {
		diffs = append(diffs, drivers.FieldDiff{Path: "spec.maximumRetryAttempts", OldValue: observed.MaximumRetryAttempts, NewValue: desired.MaximumRetryAttempts})
	}
	if !int32PtrMatch(desired.MaximumRecordAgeInSeconds, observed.MaximumRecordAgeInSeconds) {
		diffs = append(diffs, drivers.FieldDiff{Path: "spec.maximumRecordAgeInSeconds", OldValue: observed.MaximumRecordAgeInSeconds, NewValue: desired.MaximumRecordAgeInSeconds})
	}
	if desired.ParallelizationFactor != nil && *desired.ParallelizationFactor != observed.ParallelizationFactor {
		diffs = append(diffs, drivers.FieldDiff{Path: "spec.parallelizationFactor", OldValue: observed.ParallelizationFactor, NewValue: *desired.ParallelizationFactor})
	}
	if desired.TumblingWindowInSeconds != nil && *desired.TumblingWindowInSeconds != observed.TumblingWindowInSeconds {
		diffs = append(diffs, drivers.FieldDiff{Path: "spec.tumblingWindowInSeconds", OldValue: observed.TumblingWindowInSeconds, NewValue: *desired.TumblingWindowInSeconds})
	}
	if !destinationMatch(desired.DestinationConfig, observed.DestinationConfig) {
		diffs = append(diffs, drivers.FieldDiff{Path: "spec.destinationConfig", OldValue: observed.DestinationConfig, NewValue: desired.DestinationConfig})
	}
	if !scalingMatch(desired.ScalingConfig, observed.ScalingConfig) {
		diffs = append(diffs, drivers.FieldDiff{Path: "spec.scalingConfig", OldValue: observed.ScalingConfig, NewValue: desired.ScalingConfig})
	}
	if !slices.Equal(desired.FunctionResponseTypes, observed.FunctionResponseTypes) {
		diffs = append(diffs, drivers.FieldDiff{Path: "spec.functionResponseTypes", OldValue: observed.FunctionResponseTypes, NewValue: desired.FunctionResponseTypes})
	}
	return diffs
}
