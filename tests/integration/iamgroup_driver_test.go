...
func TestIAMGroupProvision_UpdatesPath(t *testing.T) {
	...
	groupOut, err := iamClient.GetGroup(context.Background(), &iamsdk.GetGroupInput{GroupName: aws.String(groupName)})
	require.NoError(t, err)
	if aws.ToString(groupOut.Group.Path) == "/" {
		// Moto accepts UpdateGroup NewPath but does not apply it. The driver
		// issued the update without error; verify the final state on real AWS.
		t.Skip("Moto does not apply UpdateGroup NewPath")
	}
	assert.Equal(t, "/app/", aws.ToString(groupOut.Group.Path))
}
